...
#pragma warning restore
        ...
        [SerializeField]
        private TMPro.TMP_Text m_Ref_InputField;
        ...
        {
            if (m_Ref_InputField != null)
            {
                switch (DateSelectionMode)
                {
                    case Dates.DateSelectionMode.SingleDate:
                        m_Ref_InputField.text = (SelectedDate.HasValue) ? SelectedDate.Date.ToString(Config.Format.DateFormat) : "";
                        break;
                    case Dates.DateSelectionMode.MultipleDates:
                        var valueCount = SelectedDates.Count(s => s.HasValue);
                        m_Ref_InputField.text = ((valueCount == 1) ? SelectedDates.First(s => s.HasValue).Date.ToString(Config.Format.DateFormat)
        ...
        public void Show(bool setPositionIfNecessary = true)
        {
            ...
            if (setPositionIfNecessary && m_Ref_InputField != null)
            {
